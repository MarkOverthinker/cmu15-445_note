...
void HashTableDirectoryPage::IncrGlobalDepth() {
  size_t directory_size = Size();
  size_t new_index = directory_size;
  for (size_t i = 0; i < directory_size; i++, new_index++) {
...
uint32_t HashTableDirectoryPage::Size() { return std::pow(2, GetGlobalDepth()); }
...
uint32_t HashTableDirectoryPage::GetBroIndex(uint32_t bucket_idx) {
  uint32_t high_bit_depth = GetLocalDepth(bucket_idx) - 1;
  return bucket_idx ^ (1 << high_bit_depth);
}
